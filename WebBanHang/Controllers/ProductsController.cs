...
namespace WebBanHang.Controllers
{
    public class ProductsController : Controller
    {
        ...
        // GET: Products
        public ActionResult ProductList(int? category, int? page, string SearchString, double min = double.MinValue,
double max = double.MaxValue)
        {
          // Lấy danh sách sản phẩm từ cơ sở dữ liệu
            var products = db.Products.AsQueryable();

            // Nếu có chuỗi tìm kiếm, lọc sản phẩm theo tên
            if (!string.IsNullOrEmpty(SearchString))
            {
                products = products.Where(p => p.NamePro.Contains(SearchString));
            }

            return View(products.ToList()); // Trả về view với danh sách sản phẩm
